...
const fileType = input => {
    ...
    if (check([0x49, 0x49, 0x2A, 0x00, 0x08, 0x00, 0x00, 0x00]) &&
    ...
    if (check([0x30, 0x30, 0x30, 0x30, 0x30, 0x30], { offset: 148, mask: [0xF8, 0xF8, 0xF8, 0xF8, 0xF8, 0xF8] }) && // Valid tar checksum
    ...
    if (check([0x66, 0x74, 0x79, 0x70], { offset: 4 }) && // `ftyp`
    ...
    for (let start = 0; start < 2 && start < (buffer.length - 16); start++) {
        ...
        if (check([0xFF, 0xE4], { offset: start, mask: [0xFF, 0xE6] }) // MPEG 1 or 2 Layer 2 header
        ) {
            return {
                ext: 'mp2',
                mime: 'audio/mpeg'
    ...
    if ((buffer[0] === 0x43 || buffer[0] === 0x46) &&
